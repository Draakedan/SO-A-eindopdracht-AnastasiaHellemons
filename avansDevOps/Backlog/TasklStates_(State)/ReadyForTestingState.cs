...
namespace avansDevOps.Backlog.TasklStates__State_
{
    public class ReadyForTestingState : IState
    {
        ...
        public ReadyForTestingState(StateCount counter)
        {
        ...
        {
            TestingState testing = new(Counter, items);
            if (user.HasRole(typeof(Tester).ToString()))
            {
                int stateCount = Counter.GetStateCount(testing.GetType().ToString());
                return (testing.MaxItems > stateCount + 1);
            }
            return false;
        }
